 ...
 *
 * Handles the playable portion of the program
 */
...
int game(int level, int difficulty) {
	...
	// Position of the different roads depending on level
	if (level == 0) {
		roadX = 14;
		roadY = -5;
	...
	// Delay prevents instant button press
	delay(1000000);
	while (1) {
		...
		// Move car up and down using button 4 and 3
		if(((getbtns() & 0x04) != 0)) {
		...
		if (timerEnd()) {
			...
			// Gradually increases the speed of the track
			...
			for (i = 0; i < nObs; i++) {
				...
				// Create a new obstacle if one has passed the screen
				if (obsY[i] > pMM) {
					notFreeSpace = 1;
					// Make sure obstacles do not spawn inside eachother
					while (notFreeSpace) {
			...
			// Start to move road if the time for the goal to appear has been reached
			if (runTime > winTime*100) {
