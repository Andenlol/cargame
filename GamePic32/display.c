/*
 * File taken from https://github.com/is1200-example-projects
 * see file COPYING for copyright
 * Modified by André Lindgren 2018-03-05
 */
...
// COPIED
void display_init() {
	DISPLAY_COMMAND_DATA_PORT &= ~DISPLAY_COMMAND_DATA_MASK; // NOT 
	delay(10);
	DISPLAY_VDD_PORT &= ~DISPLAY_VDD_MASK; // NOT
	delay(1000000);
...
void display_image(int x, unsigned char pixelMap[128][32]) {
	...
	for(i = 0; i < 4; i++) {
		...
		// André Lindgren Modified the following for loop
		for(j = 0; j < 128; j++) {
			pixelBuffer = 0;
			for (k = 0; k < 8; k++) {
